...
class ImageStore {
  ...
  private _onSaveImagesSuccess?: () => void
  ...
  saveImages(tileImagesToQuery: TileImageInfo, onStarted: () => void, onSuccess: () => void, onError: ErrorCallback) {
    this._running = true
    if (this._myQueue != null) {
  ...
  _getImagesNotInDB(tileImagesToQuery: TileImageInfo, callback: (tileImages: any[]) => void, onError: ErrorCallback) {
    const tileImagesToQueryArray: any[] = []
    ...
    this.storage.getDenseBatch(
      ...
      (tileImages) => {
        let i = 0
        const tileInfoOfImagesNotInDB: any[] = []
        this._eventEmitter.fire('tilecachingstart', null)

        this._nbTilesLeftToSave = 0
        const testTile = (tileImage: any) => {
          if (!tileImage) {
  ...
  _saveTile(data: any, callback: async.ErrorCallback<Error>) {
    // when the image is received, it is stored inside the DB using Base64 format
    const gettingImage = (response: XMLHttpRequest['response']) => {
      this.storage.put(
        data.key,
        {image: response},
        () => {
