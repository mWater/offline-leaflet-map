...
class WebSQLDataStorage implements DataStorage {
  ...
  constructor(storeName: string, onReady: () => void, onError: ErrorCallback) {
    ...
    if (window['sqlitePlugin']) {
      ...
      this._webSQLDB = window.openDatabase(
        'OfflineTileImages',
        '1.0',
        'Store tile images for OfflineLeaftMap',
        40 * 1024 * 1024,
      )
      this._webSQLDB.transaction(
        (tx) => {
  ...
  get(key: string, onSuccess: SuccessCallback, onError: ErrorCallback) {
    this._webSQLDB.transaction((tx) => {
      ...
      tx.executeSql(
        `SELECT * FROM ${this._storeName} WHERE key='${key}'`,
        [],
        onSQLSuccess,
        this._webSqlErrorHandler(onError),
      )
    })
  }
  ...
  getDenseBatch(tileImagesToQueryArray: any[], onSuccess: SuccessCallback, onError: ErrorCallback) {
    ...
    this._webSQLDB.transaction((tx) => {
      ...
      for (
        j = 0, i = j, end = tileImagesToQueryArray.length, asc = 0 <= end;
        asc ? j < end : j > end;
        asc ? j++ : j--, i = j
      ) {
      ...
      tx.executeSql(
        `SELECT * FROM ${this._storeName} WHERE key IN (${keys})`,
        [],
        onSQLSuccess,
        this._webSqlErrorHandler(onError),
      )
