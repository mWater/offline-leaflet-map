...
class OfflineLayer extends leaflet_1.TileLayer {
    ...
    _loadTile(tile, tilePoint) {
        if (!this._tileImagesStore) {
            return leaflet_1.TileLayer.prototype._loadTile.call(this, tile, tilePoint);
        }
    ...
    _getTileImages(zoomLevelLimit) {
        ...
        const roundedTileBounds = L.bounds(bounds.min.divideBy(tileSize)._floor(), bounds.max.divideBy(tileSize)._floor());
        const tilesInScreen = [];
    ...
    getTileUrl(coords) {
        ...
        return leaflet_1.Util.template(this._url, (0, leaflet_1.extend)(data, this.options));
    }
}
